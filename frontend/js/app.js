...
async function loadInfo() {
  ...
  window.info.deploymentConfig.mintPrice = await contract.methods.cost().call();
  window.info.deploymentConfig.tokensPerMint = await contract.methods.maxMintAmount().call();
  window.info.deploymentConfig.maxSupply = await contract.methods.maxSupply().call();
  ...
  if (publicMintActive) {
    ...
    try {
      const mintTransaction = await contract.methods
        .mint(amount)
        .send({ from: window.address, value: value.toString(), gasPrice: '0x174876E800' });
      if(mintTransaction) {
        if(chain === 'rinkeby') {
